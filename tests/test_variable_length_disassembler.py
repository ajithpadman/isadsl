...
def test_disassembler_file_with_variable_length(variable_length_isa_file):
    ...
    # Generate assembler and disassembler
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Check that we can disassemble both instruction types
        found_add16 = False
        found_add32 = False
        for addr, asm_str in instructions:
            if "ADD16" in asm_str.upper():
                found_add16 = True
            if "ADD32" in asm_str.upper():
                found_add32 = True
        
        assert found_add16 or found_add32, f"Expected to find ADD16 or ADD32, got {[asm for _, asm in instructions]}"
