...
{%- for instr in isa.instructions %}
    def _disassemble_{{ instr.mnemonic }}(self, instruction_word: int) -> Optional[str]:
        ...
        # Try to disassemble {{ slot.name }} using bundle instructions
        {{ slot.name }}_asm = None
        {%- if instr.bundle_instructions %}
        {%- for sub_instr in instr.bundle_instructions %}
        {%- set slot_idx = loop.index0 %}
        {%- if slot_idx < (instr.bundle_format.slots|length) %}
        # Try {{ sub_instr.mnemonic }} for {{ slot.name }}
        result_{{ slot.name }}_{{ sub_instr.mnemonic }} = self._disassemble_{{ sub_instr.mnemonic }}({{ slot.name }}_word)
        if result_{{ slot.name }}_{{ sub_instr.mnemonic }} is not None:
            {{ slot.name }}_asm = result_{{ slot.name }}_{{ sub_instr.mnemonic }}
        {%- endif %}
        {%- endfor %}
        {%- endif %}
        # If no match found, try generic disassembly
